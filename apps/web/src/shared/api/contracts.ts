...
/**
 * Surface-aware rail copy generated by the worker. Replaces the per-page
 * hardcoded title/subtitle strings the FE used to ship — see
 * `worker/src/handlers/generate_recommendation.py:_build_rail_copy`.
 *
 * `subtitle` already absorbs `personalization_reason` when present; FE
 * callsites can render `rail.subtitle` directly.
 */
export type RecommendRailCopy = {
  eyebrow: string;
  headline: string;
  subtitle: string;
  /**
   * Small chip rendered next to the headline. `null` when personalization
   * fired (the headline already signals it); a surface-tuned string when
   * generic — e.g. "Editor's picks" on home, "Top in this category" on
   * catalog, "Trending now" on an empty cart. Replaces the old hardcoded
   * "Generic mode" pill so cold-start state still feels editorial.
   */
...
export type RecommendResponse = {
  ...
  /**
   * Optional in the type so a worker that hasn't been redeployed (or any
   * old shape returning from a non-cached path) doesn't crash the rail.
   * Always present in the v2 worker output. Use `resolveRailCopy()` to
   * read it with surface-tuned FE fallbacks rather than dereferencing
   * `data.rail.*` directly.
   */
